...
@Repository("bGroupBankAccountPPO")
public class BGroupBankAccountPPO extends IProcessing {

	private static final Logger log = LoggerFactory.getLogger(BGroupBankAccountPPO.class);
	...
	public void processUpdateFormula(PostProcessJob pJob, Map<String,String> formulaMap) {
		...
		if(pJob.getRelatedObject().containsKey(Tx.class)) {
			...
			try {
				// Update Balance in Transaction for Bank Group Account for Group Loan or Investments
				if(tx.getBankGroupBankAccount() > 0 &&
					...
					GBankAccount gBankAc = pJob.getDaoFactory().getGBankAccountDAO().findById(tx.getBankGroupBankAccount());
